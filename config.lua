Config = {
    Locale = 'pl', -- this sets locale which means that its translation. There are only 'pl' and 'en'

    EnableControl = true, -- Im not 100% sure if it will override esx_ambulancejob. I'm adding this so in case of something it may help.
    ActivateControl = 46, -- this sets button that is responsible for revivivng/healing you. All of the buttons available are here https://docs.fivem.net/docs/game-references/controls/
    ...
    Money = 3000, -- this indicates how much money does it cost to get you healed
    ...
    reviveMoney = 4500, -- this is how much people will pay if they get revived its not adding to money. I know it says fee but its same as money but when you're dead
    ...
    usemotiontext = false, -- If its on true then you'll need motiontext in your resources! If you leave it on false then its going to use standard text that is inside this script.
    ...
    EMSinService = 1, -- This is how many EMS have to be in service for doc not to work
    ...
    effectstype = 'type2', -- you have 4 types of drug effects. type1, type2, type3 and type4. If you want non then just leave an empty space. It has a nil value implemented so its not going to crash your server)

    restorefood = true, -- If its on true then It will add food amount that is typed below
    restorefoodamount = 1000000, -- this is how much it adds food to players status 
    restorewater = true, -- If its on true then It will add water amount that is typed below
    restorewateramount = 1000000 -- this is how much it adds water to players status 
}
